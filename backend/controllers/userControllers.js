...
const postCourseController = async (req, res) => {
  try {
    ...
    // Set price to "free" if it's zero, otherwise use C_price
    price = C_price == 0 ? "free" : C_price;

    // Create a course instance and save it
    ...
    res
      .status(500)
      .send({ success: false, message: "Failed to create course", error: error.message });
  }
};
